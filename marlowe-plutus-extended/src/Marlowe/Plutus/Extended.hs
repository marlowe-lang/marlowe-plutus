...
module Marlowe.Plutus.Extended (
  module Marlowe.Plutus.Extended,
  module Marlowe.Plutus.Pretty,
  AccountId,
  Bound (..),
  ChoiceId (..),
  ChoiceName,
...
import Marlowe.Plutus.Semantics.Types (AccountId, Bound(..), ChoiceId(..), ChoiceName, ChosenNum, Party(..), TimeInterval, Token(..), ValueId(..), ada)
import Marlowe.Plutus.Extended.Metadata.Types (MetaData)
import Marlowe.Plutus.Pretty (Pretty (..), pretty)
...
--     by the package name (`Marlowe.Plutus.Extended`) and explicit by the
--     JSON serialization.
data Module = Module
  { metadata :: MetaData
...
instance FromJSON Value where
  ...
  parseJSON (Number n) = Constant <$> S.parseInteger "constant value" n
...
instance FromJSON Module where
  parseJSON =
    withObject
      "Module"
      ( \v -> do
          version <- S.withInteger "module version" =<< (v .: "me_version")
          if version /= 1
...
instance FromJSON Timeout where
  parseJSON (Number n) = POSIXTime <$> S.parseInteger "timeout" n
