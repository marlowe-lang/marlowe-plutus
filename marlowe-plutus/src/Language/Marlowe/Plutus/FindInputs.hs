...
module Language.Marlowe.Plutus.FindInputs (
  getAllInputs,
) where
...
import Language.Marlowe.Plutus.Semantics (TransactionInput)
import Language.Marlowe.Plutus.Semantics.Types (Case (..), Contract (..), Observation (..), State)
