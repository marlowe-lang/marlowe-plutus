...
serialiseAddress network address =
  ...
  let header x = consByteString (x H.+ H.toInteger (H.fromEnum network)) H.mempty
      ...
      encodeInteger = H.error "Staking pointers not supported." -- TODO: Add support for staking pointers, even though these may never have been used on the ledger.
   in fromBuiltin H.$
        case address of
  ...
  do
    ...
    (network,)
      ...
      H.<$> case header .&. 0xF0 of
        ...
        0x20 -> guard2 pkc scc
        0x30 -> guard2 scc scc
        0x40 -> H.fail "Staking pointers not supported." -- TODO: Add support for staking pointers, even though these may never have been used on the ledger.
        0x50 -> H.fail "Staking pointers not supported." -- TODO: Add support for staking pointers, even though these may never have been used on the ledger.
        0x60 -> guard1 pkc
        0x70 -> guard1 scc
