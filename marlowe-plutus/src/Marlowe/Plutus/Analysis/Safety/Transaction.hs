...
module Marlowe.Plutus.Analysis.Safety.Transaction (
  -- * Plutus Transactions
...
import Marlowe.Plutus.Analysis.Safety.Ledger (worstMinimumUtxo')
        ...
        consolidatePayments ps =
          let extractAddressHash (P.TxOut address _ hash _) = (address, hash)
           ...
           in [ P.TxOut address value hash Nothing
              | ah@(address, hash) <- List.nub $ extractAddressHash <$> ps
              , let value = foldMap extractValue $ List.filter ((== ah) . extractAddressHash) ps
              ]
        outPayments = consolidatePayments $ List.concatMap makePayment txOutPayments
        outPaymentDatums = List.concatMap makePaymentDatum txOutPayments
...
calcMarloweTxExBudget
  ...
  (Transaction marloweState marloweContract TransactionInput{..} output _) = flip evalStateT initSequence $ do
    ...
    let semanticsName = ValidatorName "MarloweSemantics"
        ...
        (requiredUnlockedRoles, requiredLockedRoles) = L.partition (`List.elem` openRoleLockedTokens) requiredRoles
        ...
        inRoles = do
          ...
          [TxInSpec creatorAddress (roleNFT role <> oneLovelace) Nothing Nothing | role <- requiredUnlockedRoles]
            <> [ TxInSpec{tiValue = tiValue, tiScriptEvaluationContext = ctx idx, ..}
               | (role, idx) <- List.zip requiredLockedRoles [(1 :: Int) ..]
...
--catMaybes :: forall a. [P.Maybe a] -> [a]
--catMaybes [] = []
--catMaybes (Nothing : xs) = xs
--catMaybes (Just x : xs) = x : xs
--
calcValidatorsExBudget
  :: (IsString e)
  => (MonadError e m)
...
calcValidatorsExBudget evaluationContext creatorAddress txInSpecs txOutSpecs (invalidBefore, invalidHereafter) extraSignatories (ConsolidateTxOuts consolidate) (ProvideFunding provideFunding) = do
  ...
  (txInfoInputs, validatorsEvalContext) <- do
    ...
    (txIns, validatorsEvalContext) <- do
      List.unzip <$> for txInSpecs \(TxInSpec address value possibleDatum possibleScriptEvalContext) -> do
