...
import Language.Marlowe.Plutus.Semantics (MarloweParams (MarloweParams))
import Language.Marlowe.Plutus.Semantics.Types (Contract, IntervalError (..))
import PlutusLedgerApi.V1 (CurrencySymbol (CurrencySymbol), toBuiltin, POSIXTime (POSIXTime))
import Marlowe.Testing.Common (contractGen, shrinkContract)
