        ...
        do
          ...
          (time, inputs) <-
            case unsafePerformIO $ getAllInputs slotLength mcContract Nothing of
              ...
              Left _ -> discard
          mcInput <- case List.uncons inputs of
            Just (i, _) -> pure i
            Nothing -> discard
...
inputsL :: Getter MarloweContext [Input]
inputsL = to $ txInputs . mcInput
...
requireInputs f = view inputsL >>= require "Wrong number of inputs." (f . length)
...
requireNoDeposits =
  let isDeposit (NormalInput (IDeposit _ _ _ i)) = i > 0
      ...
      isDeposit _ = False
   in view inputsL >>= "No deposits" `require` (not . any isDeposit)
...
requireIncompatibleInput =
  do
    ...
    input <- do
      inputs <- view inputsL
      case List.uncons inputs of
        Just (i, _) -> pure $ getInputContent i
    ...
    let matches IDeposit{} Deposit{} = True
        ...
        matches _ _ = False
    when (any (matches input . getAction) cs) do
      throwError "Input may be compatible with action."
