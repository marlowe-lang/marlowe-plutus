...
transactionShadowing = do
  let
    contract = Let xValId (Constant 1) (Let xValId (Constant 2) Close)
  result <- getWarning contract
  assertBool "Shadowing x" $ maybe False isTransactionShadowing result

assertionFailed :: IO ()
assertionFailed = do
  let contract = Let xValId (Constant 1) (Assert (ValueEQ (UseValue xValId) (Constant 2)) Close)
  result <- getWarning contract
...
fundsOnClose = do
  let contract =
        When
          [ Case
              (Deposit alicePk alicePk ada (Constant 100))
              (Assert (ValueEQ (AvailableMoney alicePk ada) (Constant 0)) Close)
          ]
          1699974289397
          Close
  result <- getWarning contract
