...
module Marlowe.Plutus.Testing.Service (
  -- * Testing
  handle,
  handleValues,
) where

import Marlowe.Plutus.Testing.Service.Random (generateValue)
import Marlowe.Plutus.Testing.Service.Serialization (roundtripSerialization)
import Marlowe.Plutus.Testing.Service.Types (Request (..), Response (..))

import qualified Data.Aeson as A (Result (..), Value, fromJSON, object, toJSON, (.=))
import qualified Marlowe.Plutus.Semantics as Marlowe (
  computeTransaction,
  evalObservation,
  evalValue,
  playTrace,
