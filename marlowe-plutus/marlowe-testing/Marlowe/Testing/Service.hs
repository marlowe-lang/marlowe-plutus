...
module Marlowe.Testing.Service (
  -- * Testing
  handle,
  handleValues,
) where

import Marlowe.Testing.Service.Random (generateValue)
import Marlowe.Testing.Service.Serialization (roundtripSerialization)
import Marlowe.Testing.Service.Types (Request (..), Response (..))

import qualified Data.Aeson as A (Result (..), Value, fromJSON, object, toJSON, (.=))
import qualified Language.Marlowe.Plutus.Semantics as Marlowe (
  computeTransaction,
  evalObservation,
  evalValue,
  playTrace,
