...
import Control.Concurrent.Component (mkComponent_, Component)
...
mkProbeServer :: (MonadLog m, MonadUnliftIO m) => ProbeServerDependencies -> Component m ()
mkProbeServer ProbeServerDependencies{..}  = mkComponent_ "probe-server" $ liftIO $ run port $ serve api (server probes)
