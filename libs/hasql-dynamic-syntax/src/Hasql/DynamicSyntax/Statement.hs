...
import Hasql.Statement (preparable, Statement, unpreparable)
...
buildStatement handler toResult stmt =
  if paramCount > 0
    then preparable sql params (toResult $ handleTargetResultList handler <$> compileListDecoders rows)
    else unpreparable sql params (toResult $ handleTargetResultList handler <$> compileListDecoders rows)
  where
