...
import Log (MonadLog, logAttention_, logInfo_)
import qualified Hasql.Transaction as T
import qualified Hasql.Transaction.Sessions as T
import Marlowe.Plutus.Plate (Extract (..))
import Marlowe.Plutus.Semantics.Types (ChoiceId (..), Contract, Party (..), State (..))
import Marlowe.Plutus.Semantics.Types.Address (serialiseAddress)
...
indexParties connection = do
  logInfo_ "Indexing contract parties"
  either logSessionError pure =<< liftIO (Connection.use connection (mainLoop Nothing))
  logInfo_ "Contract parties indexed"

newtype DatabaseError = DatabaseError SessionError
  deriving newtype Show

instance Exception DatabaseError

logSessionError :: (MonadIO m, MonadLog m) => SessionError -> m ()
logSessionError e = do
  logAttention_ $ "Failed to index parties: " <> T.pack (show e)
  throwIO (DatabaseError e)
