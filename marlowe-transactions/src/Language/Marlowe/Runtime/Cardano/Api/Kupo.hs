...
lookupSpendRedeemer
    :: InputIndex
    -> Redeemers
    -> Maybe (Ledger.BinaryData DijkstraEra)
...
txFromCardanoTx tx = case (C.cardanoEra @era, tx) of
  ...
  (C.DijkstraEra, C.ShelleyTx _ tx') -> TransactionDijkstra tx'
  (C.ByronEra, _) -> error "Impossible. Byron tx was removed from cardano-api"
