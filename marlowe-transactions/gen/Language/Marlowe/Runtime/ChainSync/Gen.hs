...
import Test.Gen.Cardano.Api.Typed (
  genAddressShelley,
  genPlutusScript,
  genVerificationKey, genScriptInEra, genScriptHash,
 )
...
instance Arbitrary PlutusScript where
  arbitrary = do
    C.PlutusScriptSerialised script <- hedgehog $ genPlutusScript C.PlutusScriptV2
...
--     TagAdvanceToTip -> (==)
-- 
instance Arbitrary C.AnyCardanoEra where
  arbitrary =
    elements
      [ C.AnyCardanoEra ByronEra
      , C.AnyCardanoEra ShelleyEra
      , C.AnyCardanoEra AllegraEra
      , C.AnyCardanoEra MaryEra
      , C.AnyCardanoEra AlonzoEra
      , C.AnyCardanoEra BabbageEra
      , C.AnyCardanoEra ConwayEra
      ]
...
-- genEraHistory :: Gen C.EraHistory
-- genEraHistory = do
--   C.EraHistory <$> do
...
genSlot :: Gen C.SlotNo
genSlot = C.SlotNo <$> arbitrary

genEpoch :: Gen C.EpochNo
genEpoch = C.EpochNo <$> arbitrary
