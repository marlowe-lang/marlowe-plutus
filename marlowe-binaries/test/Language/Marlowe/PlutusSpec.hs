...
import Language.Marlowe.Plutus.Merkle (MerkleizedContract (..), deepMerkleize, merkleizeInputs, dataHash)
...
import PlutusLedgerApi.Common (evaluateScriptCounting, PlutusLedgerLanguage)
...
specForScript :: String -> ScriptsInfo -> Spec
specForScript version scripts@ScriptsInfo{semanticsValidatorHash, payoutValidatorHash} = do
...
mappendAssocMapFirst
  :: forall k v
   . P.Eq k
  => PlutusTxAM.Map k v
  -> PlutusTxAM.Map k v
  -> PlutusTxAM.Map k v
mappendAssocMapFirst = PlutusTxAM.unionWith const
...
validSemanticsTransaction scripts noisy =
  do
    ...
    infoData %= mappendAssocMapFirst (PlutusTxAM.unsafeFromList merkleizations)

    -- Add the spending from the script.
    (inScript, inData) <- makeScriptInput scripts
    infoInputs <>= [inScript]
    infoData %= mappendAssocMapFirst (PlutusTxAM.unsafeFromList inData)
    ...
    infoData %= mappendAssocMapFirst (PlutusTxAM.unsafeFromList outData)
    ...
    infoData %= mappendAssocMapFirst (PlutusTxAM.unsafeFromList paymentData)
...
addNoise =
  do
    ...
    infoData %= mappendAssocMapFirst datumHashMap
...
checkDoubleInput scripts@ScriptsInfo{semanticsAddress} referencePaths =
  let modifyAfter =
        do
          ...
          infoData %= mappendAssocMapFirst (PlutusTxAM.unsafeFromList [(inDatumHash, inDatum)])
          shuffleTransaction
...
evaluateSemantics
  ...
  -- ^ The result.
evaluateSemantics ScriptsInfo{plutusVersion,semanticsValidatorBytes} d r c =
...
evaluatePayout
  ...
  -> These String LogOutput
  -- ^ The result.
evaluatePayout ScriptsInfo{plutusVersion,payoutValidatorBytes} d r c =
