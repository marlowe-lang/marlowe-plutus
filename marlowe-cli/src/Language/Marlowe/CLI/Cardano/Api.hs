...
import Cardano.Api (LedgerProtocolParameters (..), ReferenceScript)
import Cardano.Api qualified as C
...
adjustMinimumUTxO era protocol address datum origValue mRefScript = (minLovelace, value <> lovelaceToValue deficit)
  where
    ...
    minLovelace = calculateMinimumUTxO shelleyEra (unLedgerProtocolParameters protocol) txOut
...
toTxOutDatumInTx era plutusDatum = do
  let scriptData = CS.fromPlutusData . PV2.toData $ plutusDatum
      hashable = unsafeHashableScriptData scriptData
  C.TxOutDatumHash (babbageEraOnwardsToAlonzoEraOnwards era) $ hashScriptDataBytes hashable
