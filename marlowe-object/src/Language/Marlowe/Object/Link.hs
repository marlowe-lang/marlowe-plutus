...
import Cardano.Api (
  ...
  StakeAddressPointer (..),
  StakeAddressReference (..), ShelleyAddr, StakeCredential (StakeCredentialByKey, StakeCredentialByScript), fromShelleyPaymentCredential, fromShelleyStakeReference,
 )
import Cardano.Ledger.BaseTypes (Network (..))
import qualified Cardano.Ledger.BaseTypes as Ledger
import Cardano.Ledger.Credential (Ptr (..), SlotNo32 (SlotNo32))
...
import qualified Marlowe.Plutus.Semantics.Types as C
import qualified Marlowe.Plutus.Semantics.Types.Address as C
...
toPlutusStaking = \case
  ...
  StakeAddressByPointer (StakeAddressPointer (Ptr (SlotNo32 slotNo) txIx certIx)) ->
    Just $
      PV2.StakingPtr
        (fromIntegral slotNo)
