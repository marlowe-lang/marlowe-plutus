...
import Servant.API (
  Description,
  Summary, StdMethod (POST), UVerb, HasStatus, StatusOf
 )

type RoleAPI =
  "role-tokens"
  :> "burnTxs"
  :> BuildBurnTokensTxAPI -- :<|> Capture "TxId" TxId :> SubmitBurnTokensTxAPI)
...
type BuildBurnTokensTxAPI =
  Summary "Build a Burn role tokens Transation"
    ...
    :> ReqBody '[JSON] RoleTokenFilter
    :> PostTxAPI
      -- (PostCreated '[TxJSON BurnTokensTx] (BurnRoleTokensTxEnvelope CardanoTx))
      (UVerb
        'POST
        '[JSON]
        '[ BurnRoleTokensTxEnvelope CardanoTx ]
      )
...
instance HasStatus (BurnRoleTokensTxEnvelope CardanoTx) where
  type StatusOf (BurnRoleTokensTxEnvelope CardanoTx) = 201
