...
module Language.Marlowe.Runtime.Query.Database.PostgreSQL.SyntaxBuilder where
...
import PostgresqlSyntax.Rendering (toText)
...
buildStatement builder result =
  if paramCount > 1
  then preparable sql encoder result
  else unpreparable sql encoder result
  where
    sql = toText $ Rendering.preparableStmt stmt
...
instance DefaultParamEncoder Chain.TxIx where
  defaultParam = Encoders.nonNullable $ fromIntegral . Chain.unTxIx >$< Encoders.int2
