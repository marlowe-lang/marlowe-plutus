...
module Language.Marlowe.Runtime.Query.Database.PostgreSQL.GetNextSteps where
...
import Marlowe.Plutus.Semantics (MarloweData (..), MarloweParams (MarloweParams))
...
import Language.Marlowe.Runtime.History.Api (ContractStep (..), RedeemStep (..), SomeContractStep(..))
import Language.Marlowe.Runtime.Query.Database (Next (..))
import Language.Marlowe.Runtime.Query.Database.PostgreSQL.GetTip (getTip)
import qualified PlutusLedgerApi.V2 as PV2
import Witherable (catMaybes, mapMaybe)
import Prelude hiding (init)

getNextSteps :: ContractId -> ChainPoint -> T.Transaction (Next SomeContractStep)
    ...
    BeforeTip tip ->
      getNextTxIds contractId point >>= \case
        ...
        Just NextTxIds{..} -> do
          ...
          pure $ Next nextBlock tip $
              (SomeContractStep MarloweV1 . ApplyTransaction <$> applySteps)
              <> (SomeContractStep MarloweV1 . RedeemPayout <$> redeemSteps)
